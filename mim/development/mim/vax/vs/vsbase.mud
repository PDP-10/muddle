...
<INCLUDE-WHEN <COMPILING? "VSBASE"> "VSDEFS" "VSUDEFS">

<SETG SEND-PACKET <IUVECTOR 6>>
<SETG RECEIVE-PACKET <IUVECTOR 6>>
<GDECL (SEND-PACKET RECEIVE-PACKET) <UVECTOR [6 FIX]>>
...
<DEFINE VSB-SEND (VS:VS FORCE?:<OR ATOM FALSE> REPLY?:<OR ATOM FIX FALSE>
   ...
   <COND (<OR <NOT <VS-BUFFER .VS>>
	  ...
	  <COND (<AND <CHANNEL-OP .CHN WRITE-BUFFER !.STUFF> .REPLY?>
		 <VSB-WAIT .VS .REPLY?>)>)
	 (T
...
<DEFINE STUFF ST (OBJ:<OR <PRIMTYPE STRING> <PRIMTYPE UVECTOR>>
	       LEN:<OR FIX FALSE> VS:VS "AUX" (BUF:STRING <VS-BUFFER .VS>)
   ...
   <COND (<G? .LEN <LENGTH .BUF>>
	  <COND (<CHANNEL-OP .CH WRITE-BUFFER .NOBJ .LEN> .RES)>)
	 (T
	...
	<COND (<==? <SET CODE <VSI-CODE .P>> ,X-ERROR>
	       ...
	       <COND (<OR <NOT .RACT>
		     ...
		     (T
		      ...
		      <CHTYPE (<I-SPAR2 .P> <I-SPAR3 .P>
			       <NTH ,VS-ERRORS <I-SPAR2 .P>>) FALSE>)>)
	      (<==? .CODE ,X-REPLY>
	       <COND (<NOT .RACT>
		      <AGAIN>)
		     (<==? .RACT ERROR>)
...
<DEFINE VSB-REPORT-ERROR (VS:VS PACKET:UVECTOR)
   <COND (<TYPE? <INTERRUPT "VS-ERROR" .VS <I-SPAR2 .PACKET>
			    <I-SPAR3 .PACKET> <I-LPAR0 .PACKET>>
		 DISMISS>)
	 (T
	  <ERROR RANDOM-ERROR-FROM-X!-ERRORS
		 <NTH ,VS-ERRORS <I-SPAR2 .PACKET>>
		 <I-SPAR3 .PACKET>
		 <I-LPAR0 .PACKET>
		 .VS>)>>

...
<DEFINE PROCESS-EVENT PE (VS:VS P:UVECTOR "AUX" WID:FIX (SW:ANY <>)
   ...
   <COND (<==? <SET KIND <VSI-KIND .P>> ,KEY-RELEASED>
	  <RETURN <> .PE>)>
